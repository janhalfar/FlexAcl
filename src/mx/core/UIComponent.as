                         ...
                         IConstraintClient
{
    ...
    private var _aclUser:User;
    [Bindable]
    public function set aclUser(value:User):void
	{
		// propagate
		if(value!=this._aclUser) {
			this._aclUser = value;
			this.updateAcl();
			for(var i:uint=0; i<this.numChildren;i++) {
				var child:Object = this.getChildAt(i);
				if(child is UIComponent) {
					(child as UIComponent).aclUser = this._aclUser;
				}
			}
		}
	}
	public function get aclUser():User
	{
		return this._aclUser;
	}
    
    private function getAclUser():User
    {
    	if(this._aclUser) {
    		return this._aclUser;
    	} else {
    		if(this.parent && this.parent is UIComponent) {
    			return (this.parent as UIComponent).getAclUser();
    		} else {
    			return null;
    		}
    	}
    }
    protected var _aclPolicy:Policy;
    [Bindable]
    public function set aclPolicy(value:Policy):void
    {
    	if(this._aclPolicy != value) {
	    	// set values
	    	if(value) {
	    		this._aclPolicy = value;
	    	}
    		this.updateAcl();
	    }
    }
    
    public function get aclPolicy():Policy
    {
    	return this._aclPolicy;
    }
    
    private function getAclPolicy():Policy
    {
    	if(this._aclPolicy) {
    		return this._aclPolicy;
    	} else {
    		if(this.parent && this.parent is UIComponent) {
    			return (this.parent as UIComponent).getAclPolicy();
    		} else {
    			return null;
    		}
    	}
    ...
    private function updateAcl():void
    {
    	var aclUser:User = this.getAclUser();
    	var aclDelegate:IAclDelegate = this.getAclDelegate();
    	var aclPolicy:Policy;
    	if(aclDelegate) {
	    	if(aclUser) {
		    	aclPolicy = this.getAclPolicy();
		    	if(aclPolicy) {
		    		aclDelegate.updateCreate(aclUser.canCreate(aclPolicy), this);
		    		aclDelegate.updateUpdate(aclUser.canUpdate(aclPolicy), this);
		    		aclDelegate.updateDrop(aclUser.canDrop(aclPolicy), this);
		    		aclDelegate.updateExecute(aclUser.canExecute(aclPolicy), this);
		    		aclDelegate.updateRead(aclUser.canRead(aclPolicy), this);
		    	}
		    }
		    if(!(aclPolicy && aclUser)) {
				aclDelegate.updateCreate(true, this);
				aclDelegate.updateUpdate(true, this);
				aclDelegate.updateDrop(true, this);
				aclDelegate.updateExecute(true, this);
				aclDelegate.updateRead(true, this);
		    }
    	}
    }
    
    private function getAclDelegate():IAclDelegate
    {
    	if(this._aclDelegate) {
    		return this._aclDelegate;
    	} else {
    		if(this.parent && this.parent is UIComponent) {
    			return (this.parent as UIComponent).getAclDelegate();
    		} else {
    			return null;
    		}
    	}
    }
    private var _aclDelegate:IAclDelegate;
    [Bindable]
    public function set aclDelegate(value:IAclDelegate):void
	{
		if(value!=this._aclDelegate) {
			this._aclDelegate = value;
		}
	}
	public function get aclDelegate():IAclDelegate
	{
		return this._aclDelegate;
	}
